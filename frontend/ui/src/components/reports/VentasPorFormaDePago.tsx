...
import { getSalesByPaymentMethodReport } from '../../api/reports';
import { SalesByPaymentMethodReportDTO } from '../../types/reports';

const VentasPorFormaDePago: React.FC = () => {
    const { t } = useTranslation();
    const [data, setData] = useState<SalesByPaymentMethodReportDTO[]>([]);
    ...
    const fetchReport = () => {
        ...
        getSalesByPaymentMethodReport(startDateTime, endDateTime)
            .then(response => {
                setData(response);
                setError(null);
            })
            .catch(err => {
                console.error("Failed to fetch sales by payment method report", err);
    ...
    return (
        <ReportLayout title={t('reports.ventas.sub.porFormaDePago')} filterBar={filterBar}>
            {isLoading ? (
                ...
                <Table striped bordered hover responsive>
                    <thead>
                        <tr>
                            <th>{t('reports.ventas.headers.paymentMethod')}</th>
...
export default VentasPorFormaDePago;
